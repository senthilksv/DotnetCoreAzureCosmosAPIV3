...
namespace Persistence
{
    public abstract class CosmosDbRepository<T> where T : AuditableBaseEntity
    {
        ...
        protected virtual async Task<T> UpdateItemAsync(T item, string partitionKeyValue)
        {
            return await this._cosmosContainer.ReplaceItemAsync<T>(item, item.Id.ToString(), new PartitionKey(partitionKeyValue));
        }
        ...
        protected async Task<DatabaseResponse> DeleteDatabaseAsync()
        {
            return await _cosmosDatabase.DeleteAsync();
        }
    }
}
