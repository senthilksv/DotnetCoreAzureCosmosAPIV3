...
namespace Service
{
    public class PolicyService : IPolicyService
    {
        ...
        public async Task<bool> DeletePolicyAsync(Guid id, PolicyViewModel policyViewModel)
        {

            var policy = await _policyRepository.FetchAsync(id, policyViewModel.ProductType);

            if (policy != null)
            {
                _logger.LogInformation($"Policy found for delete - id: {id}");
                var result = await _policyRepository.DeleteAsync(id, new Policy());
        ...
        public async Task<GetPolicyViewModel> UpdatePolicyAsync(Guid id, PolicyViewModel policyViewModel)
        {
            ...
            if (policy != null)
            {
                ...
                var updatedPolicy = await _policyRepository.UpdateAsync(policy.Id, policy);
